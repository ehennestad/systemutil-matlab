...
This document provides guidance for developing and maintaining the System Utility Toolbox toolbox.

## 📁 Project Structure

```
systemutil-matlab/
├── src/sysutil/                                   # Main toolbox source code
│   ├── +sysutil/                                  # MATLAB package namespace
│   ├── Contents.m                                 # Toolbox contents listing
│   └── gettingStarted.m                           # Getting started guide
├── tests/                                         # Unit tests and test utilities
│   ├── +sysutil/                                  # Namespace for unit tests and test utilites
├── tools/                                         # Development and build tools
│   ├── +sysutiltools/                             # Toolbox utilities
...
**Location**: Place your main functions in `src/sysutil/+sysutil/`
...
%
% Syntax:
%   output = sysutil.myFunction(input)
%   output = sysutil.myFunction(input, options)
%
% Description:
...
% Examples:
%   % Basic usage
%   result = sysutil.myFunction(data);
%
%   % With options
%   opts.field1 = value;
%   result = sysutil.myFunction(data, opts);
%
...
addpath(fullfile(fileparts(mfilename('fullpath')), '..', 'src', 'sysutil'));
end
...
result = sysutil.myFunction(input);
...
input = testCase.TestData.sampleData;
options.field1 = 2;
result = sysutil.myFunction(input, options);
verifyClass(testCase, result, 'double');
end

function testErrorHandling(testCase)
% Test error conditions
verifyError(testCase, @() sysutil.myFunction('invalid'), ...
...
```matlab
% Package the toolbox
sysutiltools.packageToolbox()

% Install locally for testing
sysutiltools.installMatBox()
```
...
```matlab
% Check code quality
checkcode('src/sysutil/')

% Profile performance
profile on
sysutil.myFunction(data);
profile viewer

% Dependency analysis
[fList, pList] = matlab.codetools.requiredFilesAndProducts('src/sysutil');
```
...
- **Path Management**: Always use relative paths and the `sysutiltools.projectdir()` function
