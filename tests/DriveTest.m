...
% DriveTest - Unit tests for sysutil.drive functions
    ...
    methods (Test)
        ...
        function testWindowsDriveTypes(testCase)
            ...
            % Check that drive types are non-empty strings
            for i = 1:height(infoTable)
                driveType = string(infoTable.DriveType(i));
                testCase.verifyTrue(strlength(driveType) > 0, ...
                    'DriveType should not be empty');
                % Should contain one of these keywords
                testCase.verifyTrue(contains(driveType, 'Fixed') || ...
                    contains(driveType, 'Removable') || ...
                    contains(driveType, 'CD-ROM') || ...
                    contains(driveType, 'RAID') || ...
                    contains(driveType, 'Virtual'), ...
                    sprintf('DriveType "%s" should indicate drive type', driveType));
            end
        end
